...
export interface OrganizationBackgroundState {
...
export const defaults: OrganizationBackgroundState = {
...
export function getSchema(isExempt: boolean) {
  const isMaintainedForForeignFinancialInstitution = boolean();
  const isForeignBank = boolean();
  const accountActivity = !isExempt ? object().shape({
    primaryEntityAccountActivity: string()
      .required("Account activity is required"),
    expectedWithdrawalFrequency: string()
      .required("Withdraw frequency is required"),
  ...
  const additionalAccounts = !isExempt ? object().shape({
    ...
    primaryOngoingFundingSource: string().required("The scope of business is required"),
    primaryOngoingFundingSourceClarification: string().when("primaryOngoingFundingSource", {
      is: "OTHER",
      then: string()
      .trim()
      .min(2, "Must have at least 2 characters")
